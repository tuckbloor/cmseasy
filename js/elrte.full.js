...
(function($) {
	...
	$.fn.elBorderSelect = function(o) {
		...
		var style = $('<div />').elSelect({
			tpl       : '<div style="border-bottom:4px %val #000;width:100px;margin:7px 0"> </div>',
			tpls      : { '' : '%label'},
...
elRTE.prototype.ui.prototype.buttons.fullscreen = function(rte, name) {
	...
	this.command = function() {
		...
		if (f) {
			...
			wz.height(w.height() - this.delta).css('width', '100px');
...
elRTE.prototype.ui.prototype.buttons.horizontalrule = function(rte, name) {
	...
	this.src = {
		...
		bg      : $('<div />'),
		border  : $('<div />'),
		'class' : $('<input type="text" />').css('width', '100px'),
		style   : $('<input type="text" />').css('width', '100px')
	}
	...
	this.command = function() {
		...
		this.hr = n.nodeName == 'HR' ? $(n) : $(rte.doc.createElement('hr')).css({width : '100px', height : '1px'});
...
elRTE.prototype.ui.prototype.buttons.image = function(rte, name) {
	...
	this.init = function() {	
		...
		this.src = {
			main : {
				src    : $('<input type="text" />').css('width', '100px').change(preview),
				title  : $('<input type="text" />').css('width', '100px'),
				alt    : $('<input type="text" />').css('width', '100px'),
		...
		$.each(['id', 'class', 'style', 'longdesc'], function(i, name) {
			self.src.adv[name] = $('<input type="text" style="width:100px" />');
		});
		...
		$.each(
			...
			function() {
				self.src.events[this] = $('<input type="text"  style="width:100px"/>');
		});
	}
		...
		var img, 
			...
			src = fm
				? $('<div class="elrte-image-src-fm"><span class="ui-state-default ui-corner-all"><span class="ui-icon ui-icon-folder-open"/></span></div>')
					.append(this.src.main.src.css('width', '100px'))
				: this.src.main.src;
...
elRTE.prototype.ui.prototype.buttons.link = function(rte, name) {
	...
	this.command = function() {
		...
		if (this.rte.options.fmAllow && this.rte.options.fmOpen) {
			var s = $('<span />').append(this.src.main.href.css('width', '100px'))
				.append(
					$('<span />').addClass('ui-state-default ui-corner-all')
						.css({'float' : 'right', 'margin-right' : '3px'})
						.attr('title', self.rte.i18n('Open file manger'))
	...
	function init() {
		...
		$.each(self.src, function() {
			for (var n in this) {
				...
				if (t == 'INPUT' && n != 'bgimg') {
					this[n].css(this[n].attr('size') ? {'text-align' : 'right'} : {width : '100px'});
		...
		$.each(
			['onblur', 'onfocus', 'onclick', 'ondblclick', 'onmousedown', 'onmouseup', 'onmouseover', 'onmouseout', 'onmouseleave', 'onkeydown', 'onkeypress', 'onkeyup'], 
			function() {
				self.src.events[this] = $('<input type="text" />').css('width', '100px');
		});
